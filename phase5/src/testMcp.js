...
async function main() {
  ...
  if (!clientId || !clientSecret || !refreshToken) {
    ...
    for (const dir of requiredDirs) {
      console.log(`  - Folder c:\\growwpulse\\phase1\\${dir}: FOUND`);
    }
    console.log("✅ Mock connectivity check succeeded. Scaffolding for Phase 1 is complete!");
    return;
  }
  ...
  try {
    ...
    const hasGmailDraft = toolNames.some(n => n.includes("draft"));
