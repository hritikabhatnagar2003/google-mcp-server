...
1. Create a Google Cloud project, enable the Google Docs API and Gmail API, and create OAuth client credentials (Desktop app). Download the `credentials.json` file and place it in the project root.
...
The first time you perform an action the OAuth flow will open a browser to authorize the app and `token.json` will be created.

Usage
-----

- POST `/append_to_doc` — JSON body: `{ "doc_id": "<DOCUMENT_ID>", "content": "Text to append" }`
- POST `/create_email_draft` — JSON body: `{ "to": "recipient@example.com", "subject": "Subj", "body": "Message body" }`

Each endpoint will print the action and payload to the terminal and prompt: `Approve? (y/n)` — only `y` proceeds.

Notes
-----

- Do not commit `credentials.json` or `token.json`.
- `docs_tool.py` and `gmail_tool.py` use the Google API Python client libraries.
