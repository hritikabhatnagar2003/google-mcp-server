...
const { normalizeAppStore, normalizePlayStore } = require('./normalizer');
...
async function run() {
  ...
  if (isMockMode) {
    ...
    try {
      // For raw audit saving, we actually want to save raw JSON. 
      // But our fetchAppStoreReviews returns normalized. 
      // Let's modify run script to save raw data separately if we want, or just normalized logs.
      // To satisfy the requirement of saving raw to data/raw:
      // We will perform a simple direct call or save what we fetch.
      // Let's fetch App Store
      console.log('--- Step 1: Fetching App Store Reviews ---');
      const appStoreNormalized = await fetchAppStoreReviews(Math.ceil(limit / 50));
      
      // Let's save these to a raw-audit style format or mock raw
      ensureDir(RAW_DIR);
      fs.writeFileSync(path.join(RAW_DIR, 'app_store_raw.json'), JSON.stringify(appStoreNormalized, null, 2), 'utf8');
      appStoreRaw = appStoreNormalized; // in live mode, already normalized
    } catch (err) {
    ...
    try {
      ...
      playStoreRaw = playStoreNormalized; // in live mode, already normalized
    } catch (err) {
    ...
    if (appStoreRaw.length === 0 && playStoreRaw.length === 0) {
      ...
      // We need to normalize the raw mocks
  ...
  if (isMockMode) {
    ...
    // If live fetched, they are already normalized by the appStore and playStore modules
    // Just merge them
  ...
  const filteredReviews = filterByDateWindow(mergedReviews, windowWeeks);
